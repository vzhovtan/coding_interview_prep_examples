...
def find_longest_path(string):
	max_len = 0
	num_files = string.count(".")
	...
	for token in parts:
		...
		# print(levels, "<<<<< LEVELS", "\n")
		# print("levels[level]", levels[level], "=", "levels[level - 1]", levels[level - 1], "Length of token", len(token), "level", level, "\n")
		if "." in token:
